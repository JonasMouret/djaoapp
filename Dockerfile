...
RUN DEBIAN_FRONTEND=noninteractive /usr/bin/apt-get -y install make curl gnupg build-essential python3-psycopg2 python3-cryptography python3-coverage python3-setproctitle python3-lxml python3-pillow python3-cffi python3-billiard python3-ldap python3-cairo libpangoft2-1.0-0
...
RUN npm install
# RUN /app/bin/pip install --trusted-host pypi.python.org --upgrade pip
# RUN /app/bin/pip install --trusted-host pypi.python.org -r requirements.txt
...
RUN /app/bin/python manage.py migrate --run-syncdb
...
#   && ["/app/bin/gunicorn", "-c", "/etc/djaoapp/gunicorn.conf", "djaoapp.wsgi"]
# CMD ["/app/bin/gunicorn", "-c", "/etc/djaoapp/gunicorn.conf", "djaoapp.wsgi"]
# CMD ["tail", "-f", "/etc/djaoapp/gunicorn.conf"]
